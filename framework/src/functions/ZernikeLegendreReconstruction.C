...
registerMooseObject("MooseApp", ZernikeLegendreReconstruction);
...
ZernikeLegendreReconstruction::ZernikeLegendreReconstruction(const InputParameters & parameters) :
    ...
    _zernike_function(dynamic_cast<ZernikePolynomial&>(_mci_feproblem.
      getFunction(parameters.get<std::string>("zernike_function")))),
    _dbg(parameters.get<bool>("dbg"))
{
  ...
  /* The coupled variables poly_coeffs are a vector that we need to store into
     a member variable. This variable is assumed to hold all of the Zernike
     coefficients for a given Legendre order. The number of values in this
     vector will depend on the order of the Zernike polynomial, but is not as
     simple as calculating n * m, since m can only have values -n, -n + 2,
     -n + 4, ..., n. For a fixed Legendre order, there are n + 1 Zernike
     polynomials for any particular n, and 0 <= n <= N. These coefficients are
     assumed to be ordered as (n, m), such that the coefficients correspond to
     Z_0^0, Z_1^{-1}, Z_1^1, ... */
  _num = (_n_order + 1) * (_n_order + 2) / 2;
  ...
  for (int l = 0; l <= _l_order; ++l)
  {
    ...
    for (int n = 0; n <= _n_order; ++n)
    {
      for (int m = -n; m <= n; m += 2)
      {
        ...
        if (_dbg)
        {
          _console << "(l, n, m): " << l << n << m << std::endl;
          _console << "coefficient: " << (*_poly_coeffs[l])[entry] << std::endl;
          _console << "legendre: " << lfunc << std::endl;
          _console << "zernike: " << zfunc << std::endl;
          _console << "overall value: " << val << std::endl;
        }
        entry += 1;
      }
    }
